...
export const api = {
  ...
  // Sync
  syncAll: (shop) =>
    request(`/sync/all?shop=${encodeURIComponent(shop)}`, { method: "POST" }),
  syncProducts: (shop) =>
    request(`/sync/products?shop=${encodeURIComponent(shop)}`, { method: "POST" }),
  syncOrders: (shop) =>
    request(`/sync/orders?shop=${encodeURIComponent(shop)}`, { method: "POST" }),

  // Dashboard
  dashboard: (shop) =>
    request(`/dashboard?shop=${encodeURIComponent(shop)}`),

  // Products
  products: (shop) =>
    request(`/products?shop=${encodeURIComponent(shop)}`),
  ...
  recommendations: (shop) =>
    request(`/recommendations?shop=${encodeURIComponent(shop)}`),
  ...
  applyAllPrices: (shop) =>
    request(`/prices/apply-all?shop=${encodeURIComponent(shop)}`, {
      method: "POST",
    }),

  undoPrice: (shop, variantId) =>
    request(`/prices/undo/${variantId}?shop=${encodeURIComponent(shop)}`, {
      method: "POST",
    }),

  // Inventory
  inventory: (shop) =>
    request(`/inventory?shop=${encodeURIComponent(shop)}`),
  ...
  // AI
  aiSummary: (shop) =>
    request(`/ai/summary?shop=${encodeURIComponent(shop)}`, {
      method: "POST",
    }),

  aiProduct: (shop, variantId) =>
    request(`/ai/product/${variantId}?shop=${encodeURIComponent(shop)}`, {
      method: "POST",
    }),

  aiInventory: (shop, variantId) =>
    request(`/ai/inventory/${variantId}?shop=${encodeURIComponent(shop)}`, {
      method: "POST",
    }),
